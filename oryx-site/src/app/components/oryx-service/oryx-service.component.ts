...
export class OryxServiceComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.url.changes.subscribe(() => {
      ...
      this.excellUrl = this.url.first.nativeElement.innerText;
      this.loadExcelSheet('https:' + this.excellUrl);
    });
  }
  ...
  constructor(private router: Router, private contentfulService: ContentfulService, private http: HttpClient, private _elementRef : ElementRef) {
    this.router.events
  ...
  fetchOryxService() {
    this.oryxService$ = this.contentfulService.getAllEntries('oryxServicePage').pipe(
      map(oryxService => {
        const filteredItems = oryxService?.items?.filter(item => item.fields['routePath'] === this.currentPath) || [];
          ...
          contentItems.forEach((content: any) => {
            if (content.fields.features) {
              content.fields.features.forEach((feature: any) => {
                if (feature.sys && feature.sys.type === 'Link' && feature.sys.linkType === 'Entry') {
                  const entryId = feature.sys.id;
                  if (!this.referencedEntry$[entryId]) {
                    this.referencedEntry$[entryId] = this.contentfulService.getEntryById(entryId);
                  }
                }
              });
            }
  ...
  loadExcelSheet(excelSheetUrl: string) {
    this.http.get(excelSheetUrl, { responseType: 'arraybuffer' }).subscribe(response => {
      ...
      console.log(this.excelJsonData); // Log the data in the console for testing purposes
    });
  }
